...
public class SeleniumTest {
    ...
    private void runTestWithData(boolean isName, boolean isSurname, int age, boolean isParentalConsent, boolean isDoctorsPermission, String expected) {
        if(isName) getNameElement().sendKeys(getPseudoRandomName());
        if(isSurname) getSurnameElement().sendKeys(getPseudoRandomSurname());
        getBirthDateElement().sendKeys(getDateString(age));
        if(isParentalConsent) getParentalConsentElement().click();
        if(isDoctorsPermission) getDoctorsPermissionElement().click();
        clickSubmit();
